...
impl<'a> FunctionBuilder<'a> {
    ...
    /// Should be the same as using the `FuncInstBuilder`?
    ///
    /// For the implementation see `FuncInstBuilder::build`
    pub fn append_inst(&mut self, inst: Inst, terminator: bool) {
        // We only insert the Block in the layout when an instruction is added to it
        self.ensure_inserted_block();
        ...
        match &self.func.dfg.insts[inst] {
            ir::InstructionData::Jump {
                ...
                self.declare_successor(block, inst);
            }
            ...
            ir::InstructionData::Brif {
                ...
                let block_then = branch_then.block(&self.func.dfg.value_lists);
                let block_else = branch_else.block(&self.func.dfg.value_lists);

                self.declare_successor(block_then, inst);
                if block_then != block_else {
                    self.declare_successor(block_else, inst);
                }
            }
            ...
            ir::InstructionData::BranchTable { table, .. } => {
                    ...
                    .all_branches()
                {
                    ...
                    // Call `declare_block_predecessor` instead of `declare_successor` for
                    // avoiding the borrow checker.
                    self.func_ctx.ssa.declare_block_predecessor(block, inst);
                }
            }
